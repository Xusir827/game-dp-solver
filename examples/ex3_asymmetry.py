...
def main():
    out_dir = "figures/ex3_asymmetry"
    os.makedirs(out_dir, exist_ok=True)

    # Example 3: asymmetry (Player 1 has higher cap than Player 2)
    model = ResourceGameModel(
        R=20,
        C1=6,   # stronger P1
        C2=3,   # weaker P2
        gamma=0.95,
        replenishment_dist={2: 1.0},  # deterministic baseline (can also try stochastic)
    )

    V, pi1, pi2, deltas = shapley_value_iteration(model, eps=1e-5, max_iter=3000, solver="ECOS")

    print("=== Example 3 (Asymmetry) ===")
    print("iters:", len(deltas), "final delta:", deltas[-1])
    print("V range:", (min(V), max(V)))

    # Save figures
    viz.plot_value_function(V, title="V(s) - Ex3 (Asymmetry)",
                            save_path=os.path.join(out_dir, "V.png"))
    viz.plot_convergence(deltas, title="Convergence - Ex3 (Asymmetry)",
    ...
    viz.plot_rollout(states, a1, a2, rewards, title="Rollout - Ex3", save_dir=out_dir)
