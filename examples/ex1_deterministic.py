...
def main():
    out_dir = "figures/ex1_deterministic"
    os.makedirs(out_dir, exist_ok=True)

    # Example 1: deterministic replenishment
    ...
    print("=== Example 1 (Deterministic) ===")
    print("iters:", len(deltas), "final delta:", deltas[-1])
    print("V range:", (min(V), max(V)))

    # Save figures
    viz.plot_value_function(V, title="V(s) - Ex1 (Deterministic)",
                            save_path=os.path.join(out_dir, "V.png"))
    viz.plot_convergence(deltas, title="Convergence - Ex1 (Deterministic)",
    ...
    viz.plot_rollout(states, a1, a2, rewards, title="Rollout - Ex1", save_dir=out_dir)

    # close figures to avoid too many windows
    plt.close("all")
